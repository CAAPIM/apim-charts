# Layer7 API Gateway
This Chart deploys the API Gateway v11.x onward with the following `optional` subchart: hazelcast

## Bitnami Public Catalog Removal
The Bitnami subCharts have now been fully removed from the Gateway Helm Chart. Please read the release notes for [Gateway v3.0.39](./release-notes.md#3039-bitnami-subchart-removal) for more details.
...
## Optional
- Persistent Volume Provisioner (if using PVC for the Demo MySQL Statefulset)
...
The MySQL StatefulSet uses the Oracle MySQL Community Server image and provides basic MySQL functionality without the complexity of a full subchart.

**Enabling MySQL:** The MySQL StatefulSet is enabled by setting `database.create=true` in your values. This tells the Gateway to use the built-in MySQL StatefulSet instead of an external database.

#### Configuration

| Parameter                        | Description                               | Default                                                      |
| -----------------------------    | -----------------------------------       | -----------------------------------------------------------  |
| `mysql.image.repository`         | MySQL image repository | `container-registry.oracle.com/mysql/community-server`  |
| `mysql.image.tag`                | MySQL image tag | `8.4.3`  |
...
The TLS secret is created with a `pre-install` hook to prevent certificate regeneration on upgrades.
...
#### Connecting the Gateway to Redis

When using the Redis StatefulSet, configure the Gateway's Redis connection in `config.redis`:
```yaml
config:
  redis:
    enabled: true
    subChart:
      enabled: true  # Use the Redis StatefulSet
    groupName: l7GW
    commandTimeout: 5000
    connectTimeout: 10000
    auth:
...
The Gateway will automatically connect to the Redis StatefulSet pod at:
```
<release-name>-gateway-redis-0.<release-name>-gateway-redis-headless.<namespace>.svc.cluster.local:6379
```

**Note:** The Gateway Redis integration requires Gateway version 11.1.00 or later for standalone mode with authentication and TLS support.
