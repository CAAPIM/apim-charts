...
# PRODUCTION 1
STS_VALUES_YAML_FILE_REPLACED=./ssg-sts-values-env-01-replaced.yaml
cp ./ssg-sts-values-env-01.yaml $STS_VALUES_YAML_FILE_REPLACED
...
# helm install STS Gateway - configure using sed command utility for string substitution
sed -i "s/####edge-release-name/$EDGE_RELEASE_NAME/g" $STS_VALUES_YAML_FILE_REPLACED
sed -i "s/####edge-key-issuer/$EDGE_KEY_ISSUER/g" $STS_VALUES_YAML_FILE_REPLACED
sed -i "s/####edge-key-serial/$EDGE_KEY_SERIAL/g" $STS_VALUES_YAML_FILE_REPLACED
sed -i "s/####edge-key-subject/$EDGE_KEY_SUBJECT/g" $STS_VALUES_YAML_FILE_REPLACED
sed -i "s/####edge-key-cert/$EDGE_KEY_CERT_ESCAPED/g" $STS_VALUES_YAML_FILE_REPLACED
sed -i "s/####sts-release-name/$STS_RELEASE_NAME/g" $STS_VALUES_YAML_FILE_REPLACED
sed -i "s/####sts-key-issuer/$STS_KEY_ISSUER/g" $STS_VALUES_YAML_FILE_REPLACED
